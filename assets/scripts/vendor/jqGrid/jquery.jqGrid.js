	...
	return this.each( function() {
		...
		emptyRows = function (scroll, locdata) {
			...
			if (this.p.deepempty) {
				...
				$(this).find('tbody').empty().append(firstrow);
			}
...
"use strict";	
	...
	$.unformat = function (cellval,options,pos,cnt) {
		...
		//>>>override
		//zhenyong：在工厂里每次新增一行的时候都会把原生数据挂载到行对应的dom上
		else {
			var record = $(cellval).closest('tr').data('record');
			if(record && record[options.colModel.index]) {
				return record[options.colModel.index];
			}else {
				$(cellval).text();
			}
		}
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		var ahr= $("<a class='ui-jqdialog-titlebar-close ui-corner-all' style='cursor:pointer;'></a>")
...
//>>>override
		...
		stropts: [ 'eq', 'ne', 'lt', 'le', 'gt', 'ge', 'lk', 'nlk', 'in','ni'],
//<<<<override
	...
	return this.each( function() {
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			var inputAddRule = $("<input type='button' value='+' title='添加查询' class='add-rule ui-add'/>"), cm;
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			if(this.p.ruleButtons === true) {
			var ruleDeleteInput = $("<input type='button' value='-' title='删除' class='delete-rule ui-del'/>");
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				//>>> override hele
				// 打开放大镜搜索设置查询过的值时先将所有的值反编码一下，因为中文会乱码。
				var rules = defaultFilters.rules || [];
				for(var i = 0, l = rules.length; i < l; i++){
					rules[i].data = decodeURIComponent(rules[i].data);
				}
				defaultFilters.rules = rules;
				//<<< override
				found = false;
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					var ajaxOptions = $.extend({
						...
						complete:function(data,status){
							...
							///>>>override zhenyong
							$($t).triggerHandler("jqGridAddEditSubmitComplete", [postdata, $("#"+frmgr), frmoper]);
							///<<<override
							...
							if(data.status >= 300 && data.status !== 304
								...
								//>>>override 如果业务上操作失败也认为失败，不让它关闭表单
								|| (data.responseJSON && data.responseJSON.success === false)
								//<<<
								...
								//>>>override
									$($t).triggerHandler("_jqGridAddEditDelSubmitError", [data, postdata, frmoper]);
								//<<<
								...
								//>>>override
									$($t).triggerHandler("_jqGridAddEditDelSubmitSuccess", [data, postdata, frmoper]);
								//<<<
					...
					if (ret[0]) {
						if (rp_ge[$t.p.id].useDataProxy) {
							...
							var dpret = $t.p.dataProxy.call($t, ajaxOptions, "set_"+$t.p.id, postdata);
							//<<<override
							if(dpret === undefined) {
								dpret = [true, ""];
							}
							if(dpret[0] === false ) {
								...
								if(postdata.oper === opers.addoper && rp_ge[$t.p.id].closeAfterAdd ) {
								//<<<override
									$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose});
								}
								//>>>override
								// if(ajaxOptions.data.oper === opers.editoper && rp_ge[$t.p.id].closeAfterEdit ) {
								if(postdata.oper === opers.editoper && rp_ge[$t.p.id].closeAfterEdit ) {
								//<<<override
									$.jgrid.hideModal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose});
								}
							}
							...
							//>>>override
							$($t).triggerHandler("_jqGridAddEditBeforeRequest", [postdata, $("#"+frmgr), frmoper]);
							//<<<override
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					//>>>override zhenyong
					// ret = $($t).triggerHandler("jqGridDelBeforeSubmit", [postdata, $("#"+dtbl), 'del']);
					// if(ret === undefined) {
					// 	ret = [true,"",""];
					// }
					//<<<override
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						var ajaxOptions = $.extend({
							...
							complete:function(data,status){
								...
								//>>>override zhenyong
								$($t).triggerHandler("jqGridDelSubmitAfter", [postdata, $("#"+dtbl), 'del']);
								//<<<override
								...
								if(data.status >= 300 && data.status !== 304
									...
									//>>>override
									    $($t).triggerHandler("_jqGridAddEditDelSubmitError", [data, postdata, 'del', $("#"+dtbl)]);
									//<<<
									...
									//>>>override
									    $($t).triggerHandler("_jqGridAddEditDelSubmitSuccess", [data, postdata, 'del']);
									//<<<
						...
						if (ret[0]) {
							if (rp_ge[$t.p.id].useDataProxy) {
								...
								var dpret = $t.p.dataProxy.call($t, ajaxOptions, "del_"+$t.p.id, postdata); 
								//<<<override
								if(dpret === undefined) {
									dpret = [true, ""];
								}
								...
								//>>>override
								$($t).triggerHandler("_jqGridDelBeforeRequest", [postdata, $("#"+dtbl), 'del']);
								//<<<override
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				//<<<
				//>>>override
				sep = "<td class='ui-separator-wrap ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>",
				//<<<
				pgid, elemids;
				if(i===0) {
					pgid = elem;
				...
				if (o.add) {
					...
					//>>>override
					tbd = $("<td class='ui-pg-button-add ui-pg-button ui-corner-all'></td>");
					//<<<
				...
				if (o.edit) {
					tbd = $("<td class='ui-pg-button-edit ui-pg-button ui-corner-all'></td>");
					pEdit = pEdit || {};
				...
				if (o.view) {
					tbd = $("<td class='ui-pg-button-view ui-pg-button ui-corner-all'></td>");
					pView = pView || {};
				...
				if (o.del) {
					tbd = $("<td class='ui-pg-button-del ui-pg-button ui-corner-all'></td>");
					pDel = pDel || {};
				...
				if (o.search) {
					tbd = $("<td class='ui-pg-button-search ui-pg-button ui-corner-all'></td>");
					pSearch = pSearch || {};
				...
				if (o.refresh) {
					tbd = $("<td class='ui-pg-button-refresh ui-pg-button ui-corner-all'></td>");
	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			if (findnav) {
				...
				//>>>override
				p.className && tbd.addClass(p.className);
				//<<<override
