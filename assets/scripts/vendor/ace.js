...
ace.handle_side_menu = function($) {
	...
	$(document).on(ace.click_event, function (e) {
		if ($sidebar.hasClass('display') && !$sidebar.hasClass('menu-min') && !$.contains($sidebar[0], e.target)) {
...
ace.general_things = function($) {
 ...
 $('#btn-scroll-up').on(ace.click_event, function(){
	var duration = Math.min(400, Math.max(100, parseInt($(window).scrollTop() / 3)));
...
jQuery(function($) {
	...
	//this is for demo only, you can remove and have your own function, please see examples/widget.html
	//ace.widget_reload_handler(jQuery);

	/**
	 * fix some issues for tooltip
	 */
	try {
