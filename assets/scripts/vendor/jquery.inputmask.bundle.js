    ...
    if ($.fn.inputmask === undefined) {
        ...
        function maskScope(actionObj, maskset, opts) {
            ...
            function keydownEvent(e) {
                ...
                //>>>override [zhenyong]
                var oldInputValue = input._valueGet(), newInputValue;;
                //<<<override
                ...
                if (k == opts.keyCode.BACKSPACE || k == opts.keyCode.DELETE || (iphone && k == 127) || e.ctrlKey && k == 88) { //backspace/delete
                    ...
                    //>>>override [zhenyong] since 'e.preventDefault' may disable the later 'input' event
                    //here check the new input whether change, if so, trigger 'input' event
                    newInputValue = input._valueGet();

                    if(''+oldInputValue !== ''+newInputValue) {
                        // $input.trigger('input');导致兼容性BUG
                    }
                    //<<<override
            ...
            function pasteEvent(e) {
                ...
                //>>>override [zhenyong] 如果粘贴后，值不一样了，就触发input事件
                //后面返回false，导致input无法触发
                if((''+inputValue) !== (''+input._valueGet())) {
                    // $input.trigger('input'); 导致兼容性BUG
                }
                //<<<override
