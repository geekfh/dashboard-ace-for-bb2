...
var licenseNotRequired = (kind=="B2"||kind=="C1"||kind=="C2") && _.isEmpty(data.licNo);
var isWildcard = data.interMcht||data.billAddress? true:false;
var sections = [{
    caption: '身份证正面照',
    ...
    belong: 'B1,B2,D1,E1,C1,C2,branch'
},{
    caption: '身份证反面照',
    ...
    belong: 'B1,B2,D1,E1,C1,C2',
    optinal: licenseNotRequired || kind=="B1" || kind=="D1"
},{
    caption: '租赁协议的照片',
    desc: '1）请上传完整、清晰、真实的租赁协议；<br/>'+
          '2）照片支持png、jpg格式，最大不超过2M。<br/>',
    name: 'rentAgreement',
    egTitle: '租赁协议照示例',
    thumbTitle: '租赁协议',
    thumbImg: './assets/images/mcht/l_11.jpg',
    bigImg:   './assets/images/mcht/s_11.jpg',
    bigViewable: true,
    belong: 'B1,B2,D1,E1,C1,C2',
    optinal: !licenseNotRequired || kind=="D1"
},{
    caption: '拓展员与商户合影照片',
    desc: '1）请上传完整、清晰、真实的合影照片；<br/>'+
    '2）照片支持png、jpg格式，最大不超过2M。<br/>',
    name: 'operatorMcht',
    egTitle: '拓展员与商户合影照片示例',
    thumbTitle: '拓展员与商户合影照片',
    thumbImg: './assets/images/mcht/l_12.jpg',
    bigImg:   './assets/images/mcht/s_12.jpg',
    bigViewable: true,
    belong: 'B1,B2,D1,E1,C1,C2',
    optinal: true
},{
    ...
    belong: 'B1,B2,D1,E1,C1,C2',
    optinal: kind === 'B1' || kind === 'C1' || kind === 'C2' || kind=="D1" || isWildcard
},{
    caption: '店内全景照',
    ...
    caption: '授权证书照片',
    desc: '1）请上传完整、清晰、真实的授权证书；<br/>'+
    '2）照片支持png、jpg格式，最大不超过2M。<br/>',
    name: 'clearProtocol',
    egTitle: '授权证书示例',
    thumbTitle: '授权证书',
    thumbImg: './assets/images/mcht/clearProtocol-small.jpg',
    bigImg:   './assets/images/mcht/clearProtocol-big.jpg',
    bigViewable: true,
    belong: 'C1',
    optinal: true
}];
        ...
        <div class="pull-left section <%=item.name%>-section" belong="<%=item.belong%>">
            ...
            <div class="upload-section row" name="<%=item.name%>">
                <div class="section-left col-xs-6">
                    <div class="upload-trigger <%= item.optinal===true ? 'optinal' : '' %>" <%=item.atLeastOne === true ? 'atLeastOne' : '' %>>
						<div class="btn-panel">
				...
				<div class="section-right prompt-wrap col-xs-6">
					...
					<div class="example-image-wrap">
						<% if(item.warmImgArr) { %>
							<% _.each(item.warmImgArr, function (imgInfo) { %>
							<div class="view-accout-bankcard">
								<img data-fancybox-group="gallery" src="<%=imgInfo.thumbImg%>">
								<%if(imgInfo.bigViewable){%>
									<a class="view-example-trigger fancybox" href="<%=imgInfo.bigImg%>">查看大图</a>
								<%}%>
							</div>
							<% }); %>
