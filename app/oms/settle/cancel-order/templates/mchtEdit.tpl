...
%>
<div class="mcht-edit-cancelorder">
    ...
    <%if(type !== '3'){%>
        <div class="form-group">
            ...
            <div class="col-xs-7">
                <select name="contactStatus" class="form-control"  style="margin:4px auto;">
                    <%_.each(CONTACT_STATUS, function(value,key){%>
                    <option value="<%=key%>" <%=key === contactStatus ? selected="selected" : ''%>><%=value%></option>
                    <%});%>
                </select>
            </div>
        </div>
    ...
    <%if(type === '2' || type === '5'){%>
        <div class="form-group">
            ...
            <div class="col-xs-7">
                <select name="cancelReason" class="form-control"  style="margin:4px auto;">
                    <%_.each(CANCEL_REASON_CODE[type], function(value,key){%>
                    <option value="<%=key%>" <%=key === cancelReasonCode ? selected="selected" : ''%>><%=value%></option>
                    <%});%>
                </select>
            </div>
        </div>
    <%}if(type === '2'){%>
        <div class="form-group">
            ...
            <div class="col-xs-7">
                <select name="cancelType" class="form-control"  style="margin:4px auto;">
                    <%_.each(CANCEL_TYPE, function(value,key){%>
                    <option value="<%=key%>" <%=key === cancelType ? selected="selected" : ''%>><%=value%></option>
                    <%});%>
                </select>
            </div>
        </div>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-7">
            <select class="form-control" name="dealStatus" style="margin:4px auto;">
                <%_.each(DEAL_STATUS[type], function(value,key){%>
                <option value="<%=key%>" <%=key === dealStatus ? selected="selected" : ''%>><%=value%></option>
                <%});%>
            </select>
        </div>
    </div>
