...
define([
	...
	'app/oms/route/withhold/AbstarctField',
	'bootstrap-tagsinput',
	'jquery.inputmask',
	'select2',
	'common-ui'
	...
	var FieldBankView = AbstarctField.extend({
		...
		setMulValue: function (val) {
			if(val){
				...
				for(var i=0; i<keyArr.length; i++) {
					arr.push({
						key: keyArr[i],
						value: valueArr == '' ? '' : valueArr[i] //loadBankCodeByBankName(keyArr[i])
					});
				}
		...
		setupSingleVal: function () {
			var me = this;
			this.$singleInput = $('<input name="single" class="single-input">').appendTo(this.ui.singleValWrap);
			this.$singleInput.select2($.extend({width: 150, placeholder: ' - 选择支持银行 - '}, this.select2Config));
			me.$singleInput.on('change', function () {
				me.$singleInput.trigger('keyup');
			});
		},
		...
		setupMulVal: function () {
			var me = this;
			this.$mulInput = $('<input name="mul" class="mul-input">').appendTo(this.ui.mulValWrap);
			this.$mulInput.select2($.extend({multiple: true, width: 200, placeholder: '选择支持银行'}, this.select2Config));
			me.$mulInput.on('change', function () {
				me.$mulInput.trigger('keyup');
			});
		},
	...
	var FieldServiceTypeView = AbstarctField.extend({
		label: '验证要素',
		name: 'service_type',
		extraOprs: ['in'],
		select2Data: [{id: '1001', text: '三要素验证'}, {id: '1002', text: '四要素验证'}, {id: '1003', text: '二要素验证(无照片)'}, {id: '1004', text: '二要素验证(有照片)'}],
		setMulValue: function (val) {
			this.$mulInput.select2('val', val.split(','));
		},
		setSingleValue: function (val) {
		...
		setupSingleVal: function () {
			var me = this;
			this.$singleInput = $('<input name="single" class="single-input">').appendTo(this.ui.singleValWrap);
			this.$singleInput.select2({ data: this.select2Data, width: 150, placeholder: ' - 选择验证要素 - '});
			me.$singleInput.on('change', function () {
				me.$singleInput.trigger('keyup');
			});
		},
	...
	var fieldMap = [
		...
		{type:'amount',      label:'单笔交易金额(元)',fieldView:FieldAccountView},
		...
		{type:'card_bank',   label:'支持银行',fieldView:FieldBankView},
		...
		{type:'service_type',label:'验证要素',fieldView:FieldServiceTypeView}
	];
	...
	function loadBankCodeByBankName(id){
		var bankName = '';
		Opf.ajax({
			async: false,
			type: 'GET',
			url: url._('bank.bankCode.by.bankName'),
			data: { value: id },
			success: function(resp){
				if(resp){
